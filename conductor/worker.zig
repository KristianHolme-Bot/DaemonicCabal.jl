...
pub const Worker = struct {
    ...
    pub fn spawn(allocator: Allocator, io: Io, cfg: *const config.Config, id: u32, runtime_dir: []const u8, julia_channel: ?[]const u8, threads: args.Threads, interactive: bool) !Worker {
        return spawnImpl(allocator, io, cfg, id, runtime_dir, julia_channel, threads, interactive, false, null, &.{}, &.{});
    }

    pub fn spawnSandboxed(allocator: Allocator, io: Io, cfg: *const config.Config, id: u32, runtime_dir: []const u8, julia_channel: ?[]const u8, threads: args.Threads, environ_map: *const std.process.Environ.Map, extra_ro_binds: []const []const u8, extra_rw_binds: []const []const u8) !Worker {
    ...
    fn spawnImpl(allocator: Allocator, io: Io, cfg: *const config.Config, id: u32, runtime_dir: []const u8, julia_channel: ?[]const u8, threads: args.Threads, interactive: bool, sandboxed: bool, environ_map: ?*const std.process.Environ.Map, extra_ro_binds: []const []const u8, extra_rw_binds: []const []const u8) !Worker {
        ...
        const effective_runtime_dir = if (sandboxed and builtin.os.tag == .linux) blk: {
            const subdir = std.fmt.bufPrint(&subdir_buf, "{s}/sandbox-{d}", .{ runtime_dir, id }) catch return error.PathTooLong;
        ...
        const eval_expr = try std.fmt.allocPrint(allocator, "using DaemonWorker; DaemonWorker.runworker(\"{s}\", {d}, \"{s}\")", .{ setup.addr, id, cfg.socket_path });
        ...
        if (sandboxed and builtin.os.tag == .linux) {
            const exe_path = if (std.mem.indexOfScalar(u8, cfg.worker_executable, '/') != null) cfg.worker_executable else if (environ_map.?.get("PATH")) |p| resolveInPath(io, cfg.worker_executable, p) orelse cfg.worker_executable else cfg.worker_executable;
            ...
            const project_arg: ?[]const u8 = if (cfg.worker_project.len > 0) try std.fmt.allocPrint(allocator, "--project={s}", .{cfg.worker_project}) else null;
            ...
            child = try std.process.spawn(io, .{ .argv = argv.items, .pgid = if (builtin.os.tag == .windows) null else 0 });
        }
        ...
        return .{ .allocator = allocator, .id = id, .process = child, .socket = socket, .project = null, .julia_channel = julia_channel, .threads = threads, .session_label = null, .created_at = now, .last_active = now, .last_pinged = now, .active_clients = 0, .sandboxed = sandboxed, .interactive = interactive };
    }
    ...
    const Header = struct { msg_type: protocol.worker.MessageType, payload_len: u16, raw: [3]u8 };

    fn readHeader(self: *Worker) !Header {
        var buf: [3]u8 = undefined;
        try readExact(self.socket, &buf);
        return .{ .msg_type = @enumFromInt(buf[0]), .payload_len = std.mem.readInt(u16, buf[1..3], .little), .raw = buf };
    }
    ...
    pub const SocketPaths = struct { stdin: []const u8, stdout: []const u8, stderr: []const u8, signals: []const u8 };

    pub fn runClient(self: *Worker, allocator: Allocator, client_info: *const ClientInfo) !SocketPaths {
        ...
        if (header.msg_type == .err) {
            ...
            if (header.payload_len > 0 and header.payload_len < 4096) {
                const err_payload = allocator.alloc(u8, header.payload_len) catch return error.WorkerError;
                defer allocator.free(err_payload);
                readExact(self.socket, err_payload) catch return error.WorkerError;
